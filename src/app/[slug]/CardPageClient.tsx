...
export default function CardPageClient({ slug, initialCard }: Props) {
  ...
  useEffect(() => {
    ...
    if (initialCard) {
      trackCardView(slug, initialCard.nombre)
      return
    }
        ...
        <div className="flex min-h-screen items-center justify-center bg-mts-bg px-4">
          <div className="w-full max-w-sm space-y-5 text-center">
            ...
            <p className="text-sm leading-relaxed text-mts-muted">
              Esta tarjeta aún no ha sido creada. Ve al{' '}
              <span className="font-medium text-indigo-400">seed</span> para crearla.
            </p>
            <div className="flex flex-col gap-2">
              ...
              <Link href="/" className="text-sm text-mts-muted transition-colors hover:text-white">
                ← Volver al inicio
              </Link>
            </div>
          </div>
